...
class LLMJudge:
    def __init__(self, models: Optional[List[str]] = None):
        # gpt-4o-mini = fast screener (cheaper)
        # gpt-4o      = authoritative judge (medical accuracy & safety critical)
        self.models = models or ["gpt-4o-mini", "gpt-4o"]
        # Tiebreaker defers to stronger model — medical domain requires it
        self.tiebreaker_model = "gpt-4o"
    ...
    async def _call_openai(self, model: str, user_prompt: str) -> Dict[str, Any]:
    ...
    async def _call_model(self, model: str, user_prompt: str) -> Dict[str, Any]:
        # All models use OpenAI API — extend here if needed in future
    ...
    async def _resolve_conflict(
        ...
        try:
            # Always use gpt-4o as tiebreaker — safety-critical domain
            tiebreaker = await self._call_openai(self.tiebreaker_model, user_prompt)
            return tiebreaker
        except Exception as e:
            logger.warning(f"Tiebreaker failed, using conservative fallback: {e}")
            # In medical context: take the LOWER score (more conservative)
            conservative_acc  = min(result_a.get("accuracy", 3),       result_b.get("accuracy", 3))
            conservative_prof = min(result_a.get("professionalism", 3), result_b.get("professionalism", 3))
            safety_a    = result_a.get("safety", "pass")
            safety_b    = result_b.get("safety", "pass")
            # If either judge flags safety — fail (conservative)
            final_safety = "fail" if safety_a == "fail" or safety_b == "fail" else "pass"
            return {
                "accuracy":        conservative_acc,
                "professionalism": conservative_prof,
                "safety":          final_safety,
                "reasoning":       "Conservative fallback due to tiebreaker failure",
    ...
    async def evaluate_multi_judge(
        ...
        if self._has_conflict(result_a, result_b):
            ...
            final_score = float(all_acc[1])  # median of 3
        else:
            # No conflict: weight gpt-4o score higher (60/40) — it's the authority
            final_score = round(score_a * 0.4 + score_b * 0.6, 2)
        ...
        reasoning_parts   = [
            f"{model_a} (screener): {result_a.get('reasoning', 'N/A')}",
            f"{model_b} (authority): {result_b.get('reasoning', 'N/A')}",
        ]
    ...
    async def check_position_bias(
        ...
        prompt_original = _build_user_prompt(
            "Compare these two responses", response_a, response_b
        )
        prompt_swapped = _build_user_prompt(
            "Compare these two responses", response_b, response_a
        )

        # Use stronger model for bias check — medical context
        model = "gpt-4o"
        ...
        if isinstance(original_result, Exception) or isinstance(swapped_result, Exception):
            return {
                ...
                "score_swapped":  None,
                "error": str(
                    original_result if isinstance(original_result, Exception) else swapped_result
                ),
            }
